...
public class DriverFactory {
	...
	public static synchronized WebDriver getBrowser (BrowserType browserType, ExecutionType executionType) {
        ...
        boolean googleChrome = browserType == BrowserType.GOOGLE_CHROME || (browserType == BrowserType.FROM_EXCEL && DriverHelper.BROWSER_TYPE.equalsIgnoreCase("google chrome"));
        boolean mozillaFirefox = browserType == BrowserType.MOZILLA_FIREFOX || (browserType == BrowserType.FROM_EXCEL && DriverHelper.BROWSER_TYPE.equalsIgnoreCase("mozilla firefox"));
        boolean microsoftEdge = browserType == BrowserType.MICROSOFT_EDGE || (browserType == BrowserType.FROM_EXCEL && DriverHelper.BROWSER_TYPE.equalsIgnoreCase("microsoft edge"));
        String browserTypeWarningMsg = "The driver is null! because the browser type [" + DriverHelper.BROWSER_TYPE + "] is not valid/supported; Please choose a valid browser type from the given choices in the properties file";
		// Remote execution condition
        if (executionType == ExecutionType.REMOTE || (executionType == ExecutionType.FROM_EXCEL && DriverHelper.BROWSER_TYPE.equalsIgnoreCase("remote"))) {
			if ( googleChrome ) {
				try {
                ...
                CustomReporter.logMessage(browserTypeWarningMsg);
				fail(browserTypeWarningMsg);
//		throw new NullPointerException(warningMsg);
			}
		}
		// Local execution condition
        else if (executionType == ExecutionType.LOCAL || (executionType == ExecutionType.FROM_EXCEL && DriverHelper.EXECUTION_TYPE.equalsIgnoreCase("local"))) {
			if ( googleChrome ) {
...
//		throw new NullPointerException(warningMsg);
		...
		// Local Headless execution condition
        else if (executionType == ExecutionType.LOCAL_HEADLESS || (executionType == ExecutionType.FROM_EXCEL && DriverHelper.EXECUTION_TYPE.equalsIgnoreCase("local headless"))) {
			if ( googleChrome ) {
                //WebDriverManager.chromedriver().setup();
            ...
            String warningMsg = "The driver is null! because the execution type [" + DriverHelper.EXECUTION_TYPE + "] is not valid/supported; Please choose a valid execution type from the given choices in the properties file";
            CustomReporter.logMessage(warningMsg);
			fail(warningMsg);
		}
		// start session
		return driver.get();
