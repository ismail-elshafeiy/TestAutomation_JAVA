...
public class FileActions {
            ...
            for (Enumeration<JarEntry> e = jarFile.entries(); e.hasMoreElements(); ) {
                ...
                if (jarEntryName.startsWith(jarConnectionEntryName)) {
                    ...
                    if (jarEntry.isDirectory()) {
                        ...
                        if (success) {
                            CustomReporter.logMessage("Directory Created successfully...");
                        }
                    } else {
    ...
    public boolean zipFiles(String srcFolder, String destZipFile) {
        ...
        try (var fileWalker = Files.walk(Paths.get(srcFolder))) {
            CustomReporter.logMessage("Archiving the following files:\n");
    ...
    public URL downloadFile(String targetFileURL, String destinationFilePath, int connectionTimeout,
        ...
        if (targetFileURL != null && destinationFilePath != null) {
            try {
                CustomReporter.logMessage("Downloading a file from this url \"" + targetFileURL + "\" to this directory \""
                ...
                FileUtils.copyURLToFile(new URL(targetFileURL), new File(destinationFilePath), connectionTimeout,
                        readTimeout);
                CustomReporter.logMessage("Downloading completed successfully.");
    ...
    private String reportActionResult(String actionName, String testData, String log, Boolean passFailStatus, Exception... rootCauseException) {
        ...
        if (parentMethod.getClassName().contains("shaft")) {
            ...
            if (!attachments.equals(new ArrayList<>())) {
                CustomReporter.logMessage(attachments.toString());
            } else {
                CustomReporter.logMessage(message);
            }
        }
